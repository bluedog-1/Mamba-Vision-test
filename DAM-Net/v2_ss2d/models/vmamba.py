...
class VSSLayer(nn.Module):
    def __init__(self, dim=96, drop_path=[0.1, 0.1], use_checkpoint=False,
                 norm_layer=nn.LayerNorm, downsample=nn.Identity(), channel_first=False,
                 ...
                 mlp_drop_rate=0.0, gmlp=False, _SS2D=SS2D, **kwargs):
        super().__init__()
        self.blocks = nn.ModuleList()
        self.downsample = downsample
        self.use_checkpoint = use_checkpoint
        self.depth = len(drop_path)

        self.da_modules = nn.ModuleList()
        for i in range(self.depth):
            self.da_modules.append(DAModule(hidden_dim=dim, num_inputs=i + 1))
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i in range(self.depth):
            Xs_stack = torch.stack(Xs_list, dim=0)    # [L, B, C, H, W]
            agg_input = self.da_modules[i](Xs_stack)
            if self.use_checkpoint:
                out = checkpoint.checkpoint(self.blocks[i], agg_input)
            else:
                out = self.blocks[i](agg_input)
            # 不要再 clone, 直接 append
            Xs_list.append(out)
        
        return self.downsample(Xs_list[-1])

class DAModule(nn.Module):
    def __init__(self, hidden_dim, num_inputs, eps=1e-6):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.num_inputs = num_inputs
        self.eps = eps

        self.norm = RMSNorm2D(hidden_dim)
        self.mlp = nn.Sequential(
            nn.Conv2d(hidden_dim, 4 * num_inputs, kernel_size=1),
            nn.GELU(),
            nn.Conv2d(4 * num_inputs, num_inputs, kernel_size=1)
        )

        # 直接定义成 (1, num_inputs, 1, 1)，不用在 forward 里再 view
        self.static_weights = nn.Parameter(torch.zeros(1, num_inputs, 1, 1))
        with torch.no_grad():
            self.static_weights[0, -1, 0, 0] = 1.0

    def forward(self, Xs):  # Xs: [L, B, C, H, W]
        # Xs: 层级堆叠的所有历史输出
        L, B, C, H, W = Xs.shape

        # 归一化上一层输出
        x_latest = self.norm(Xs[-1])            # [B, C, H, W]
        dynamic_logits = self.mlp(x_latest)     # [B, L, H, W]
        
        # static_weights 已经是 [1, L, 1, 1]，直接广播相加
        weights = dynamic_logits + self.static_weights
        # softmax 后确保连续
        weights = torch.softmax(weights, dim=1)  # [B, L, H, W]
        # 堆叠所有层的输出，并确保连续
        X_stack = Xs.transpose(0, 1)          # [B, L, C, H, W]

        # 扩展权重维度，计算加权和
        w2 = weights.unsqueeze(2)              # [B, L, 1, H, W]
        out = (X_stack * w2).sum(dim=1)         # [B, C, H, W]
...
class RMSNorm2D(nn.Module):
    ...
    def forward(self, x):  # x: [B, C, H, W]
        norm = x.pow(2).mean(dim=1, keepdim=True).sqrt() * self.scale
        return x / (norm + self.eps) * self.weight[:, None, None]
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.pos_embed = RelativePositionEncoding(channel=dims[0],radius_bins=100,image_size=imgsize,patch_size=patch_size) if posembed else None
        ...
        with torch.no_grad():
            pe_layers = list(self.patch_embed.modules())
            if isinstance(self.patch_embed, nn.Sequential) and len(pe_layers) >= 6:
                conv = self.patch_embed[5]
                if isinstance(conv, nn.Conv2d) and not conv.weight.is_contiguous():
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.pos_embed is not None:
            ...
            pos_embed = self.pos_embed().unsqueeze(0) if not self.channel_first else self.pos_embed().unsqueeze(0).permute(0,3,1,2)
...
class RelativePositionEncoding(nn.Module):
    def __init__(self, channel: int, radius_bins: int = 100, image_size: int = 224, patch_size: int = 4):
        super().__init__()
        ...
        # 计算插值权重（使用可微操作）
        idx_floor = normalized_radius.floor()
        idx_ceil = normalized_radius.ceil()
        weight_ceil = normalized_radius - idx_floor
        weight_floor = 1 - weight_ceil
        
        # 安全索引（确保不越界）
        idx_floor = idx_floor.clamp(0, self.radius_bins - 1).long()
        idx_ceil = idx_ceil.clamp(0, self.radius_bins - 1).long()
        
        # 位置嵌入表
        self.radius_emb = nn.Embedding(self.radius_bins + 1, self.channel)  # +1 确保安全索引
        nn.init.trunc_normal_(self.radius_emb.weight, std=0.02)
        
        # 注册为buffer，确保设备同步
        self.register_buffer("idx_floor", idx_floor)
        self.register_buffer("idx_ceil", idx_ceil)
        self.register_buffer("weight_floor", weight_floor)
        self.register_buffer("weight_ceil", weight_ceil)
    
    def forward(self) -> torch.Tensor:
        # 获取嵌入向量
        emb_floor = self.radius_emb(self.idx_floor)
        emb_ceil = self.radius_emb(self.idx_ceil)
        
        # 双线性插值获取位置编码
        pos_enc = (
            self.weight_floor.unsqueeze(-1) * emb_floor +
            self.weight_ceil.unsqueeze(-1) * emb_ceil
        )
        return pos_enc # (channel, H, W)
...
def vmamba_base_s2l15(channel_first=True):
    return VSSM(
        ...
        patch_norm=True, norm_layer=("ln2d" if channel_first else "ln"), 
        downsample_version="v3", patchembed_version="v2", 
        use_checkpoint=False, posembed=True, imgsize=224, 
    )
...
# %%
    print(model_ref)
# %%
    checkpoint=torch.load('/home/shulab/wyc/Vmamba_v1/vssm1_tiny_0230s_ckpt_epoch_264.pth')
    for name , p in checkpoint['model'].items():
        print(name,p.shape)
# %%
