...
from models import *  # 自定义的 MoeMambavision 模型定义
import math
import timm.data
...
def parse_args():
    ...
    parser.add_argument('--lr', default=3e-4, type=float)      # 基准学习率，调整为ViT常用值
    parser.add_argument('--min_lr', default=1e-6, type=float)
    parser.add_argument('--max_lr', default=0.0015, type=float, help='最大学习率上限')
...
def main_worker(local_rank, args):
    ...
    if args.resume and os.path.isfile(args.resume):
        ...
        # target_model=model.module if hasattr(model,'module') else model
        # for name,params in target_model.named_children():
        #     if 'fusion' not in name and name in checkpoint['model']:
        #         params.data.copy_(checkpoint['model'][name].data)
    ...
    try:
        for epoch in range(start_epoch, args.epochs + 1):           
            ...
            if rank == 0:
                ...
                # if hasattr(model, 'module'):
                #     for i,layer in enumerate(model.module.levels):
                #         if len(layer.moe_blocks)==0:
                #             continue
                #         print(f'layer:{i}')
                #         for j,block in enumerate(layer.moe_blocks):
                #             util=block.current_batch_utilization().cpu().numpy()
                #             print(f" block{j}: Expert  = mamba {util[0]:.4f} | 'attention '{util[1]:.4f} ")
                #             logging.info(f" block{j}: Expert  = mamba {util[0]:.4f} | 'attention '{util[1]:.4f} ")
                            
                # else:
                #     for i,layer in enumerate(model.levels):
                #         if len(layer.moe_blocks)==0:
                #             continue
                #         print(f'layer:{i}')
                #         for j,block in enumerate(layer.moe_blocks):
