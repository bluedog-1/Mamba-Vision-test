...
class BiMambaVision(nn.Module):
    def __init__(
        ...
        self.A_log_forward = nn.Parameter(A_log.clone())
        self.A_log_forward._no_weight_decay = True
        self.x_proj_forward = nn.Linear(
            self.d_inner//2, self.dt_rank + self.d_state * 2, bias=False, **factory_kwargs
        )
        self.dt_proj_forward = nn.Linear(self.dt_rank, self.d_inner//2, bias=True, **factory_kwargs)
        
        
        self.A_log_backward = nn.Parameter(A_log.clone())
        self.A_log_backward._no_weight_decay = True
        self.x_proj_backward= nn.Linear(
            self.d_inner//2, self.dt_rank + self.d_state * 2, bias=False, **factory_kwargs
        )
        self.dt_proj_backward = nn.Linear(self.dt_rank, self.d_inner//2, bias=True, **factory_kwargs)
        
        
        dt_init_std = self.dt_rank**-0.5 * dt_scale
        if dt_init == "constant":
            nn.init.constant_(self.dt_proj_forward.weight, dt_init_std)
            nn.init.constant_(self.dt_proj_backward.weight, dt_init_std)
        elif dt_init == "random":
            nn.init.uniform_(self.dt_proj_forward.weight, -dt_init_std, dt_init_std)
            nn.init.uniform_(self.dt_proj_backward.weight, -dt_init_std, dt_init_std)
        else:
        ...
        with torch.no_grad():
            self.dt_proj_forward.bias.copy_(inv_dt)
            self.dt_proj_backward.bias.copy_(inv_dt)
        self.dt_proj_forward.bias._no_reinit = True
        self.dt_proj_backward.bias._no_reinit = True
    ...
    def forward(self, x):
        ...
        """
        _, seqlen, _ = x.shape
        xz = self.in_proj(x)#(B,L,D)->(B,L,d_inner)
        xz = rearrange(xz, "b l d_inner -> b d_inner l").contiguous()
        x, z = xz.chunk(2, dim=1) #(B,d_inner,L)->(B,d_inner/2,L),(B,d_inner/2,L)
        x = F.silu(self.conv1d_x(x))
        z = F.silu(self.conv1d_z(z))
        A_forward = -torch.exp(self.A_log_forward.float())
        x_dbl_forward = self.x_proj_forward(rearrange(x, "b n l -> (b l) n")).contiguous()#(B,d_inner/2,L)->(B*L,d_inner/2)->(B*L,dt+2*d_state)
        dt_forward, B_forward, C_forward = torch.split(x_dbl_forward, [self.dt_rank, self.d_state, self.d_state], dim=-1)#(B*L,dt) (B*L,d_state) (B*L,d_state)
        dt_forward = rearrange(self.dt_proj_forward(dt_forward), "(b l) d -> b d l", l=seqlen)#(B*L,dt)->(B*L,d_inner/2)->(B,d_inner/2,L)
        B_forward = rearrange(B_forward, "(b l) dstate -> b dstate l", l=seqlen).contiguous()#(B*L,d_state)->(B,d_state,L)
        C_forward = rearrange(C_forward, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
        y_forward = selective_scan_fn(x, #(B,d_inner/2,L)
                            dt_forward, #(B,d_inner/2,L)
                            A_forward, # (d_inner/2, d_state)
                            B_forward, #(B,d_inner/2,L)
                            C_forward, #(B,d_inner/2,L)
                            self.D.float(), #(d_inner/2)
                            z=None, 
                            delta_bias=self.dt_proj_forward.bias.float(), 
                            delta_softplus=True, 
                            return_last_state=None)
        
        A_backward = -torch.exp(self.A_log_backward.float())
        x_dbl_backward = self.x_proj_backward(rearrange(x, "b n l -> (b l) n")).contiguous()#(B,d_inner/2,L)->(B*L,d_inner/2)->(B*L,dt+2*d_state)
        dt_backward, B_backward, C_backward = torch.split(x_dbl_backward, [self.dt_rank, self.d_state, self.d_state], dim=-1)#(B*L,dt) (B*L,d_state) (B*L,d_state)
        dt_backward = rearrange(self.dt_proj_backward(dt_backward), "(b l) d -> b d l", l=seqlen).contiguous()#(B*L,dt)->(B*L,d_inner/2)->(B,d_inner/2,L)
        B_backward = rearrange(B_backward, "(b l) dstate -> b dstate l", l=seqlen).contiguous()#(B*L,d_state)->(B,d_state,L)
        C_backward = rearrange(C_backward, "(b l) dstate -> b dstate l", l=seqlen).contiguous()
        y_backward = selective_scan_fn(x.flip(-1), #(B,d_inner/2,L)
                            dt_backward, #(B,d_inner/2,L)
                            A_backward, # (d_inner/2, d_state)
                            B_backward, #(B,d_inner/2,L)
                            C_backward, #(B,d_inner/2,L)
                            self.D.float(), #(d_inner/2)
                            z=None, 
                            delta_bias=self.dt_proj_backward.bias.float(), 
                            delta_softplus=True, 
                            return_last_state=None)
        y=y_forward + y_backward.flip(-1)
...
class SimpleMoEBlock(nn.Module):
    def __init__(self, 
        ...
        # 专家模块
        self.mamba_expert = BiMambaVision(d_model=dim, d_state=8, d_conv=3, expand=1)
